...
///////////////////////// function prototypes
//static void printv(float *v, int listLength);
int countDigits(int num);
static int test_zero(float *newelement, int numfeatures);
static void compute_mean(t_onlineClusterKNN *x, int k, t_instance newelement);
static float squared_euclid(float *v1, float *v2, int numfeatures);
...
static void onlineClusterKNN_clear(t_onlineClusterKNN *x)
{
    ...
    if (x->randomFlag == 1)
    {
        ...
        for(i=0; i<x->numInstances; i++)
            t_freebytes_(x->instances[i].instance, x->instanceFeatureLengths[i]*sizeof(float));
        
        for(i=0; i<x->numClusters; i++)
            t_freebytes_(x->means[i].instance, x->instanceFeatureLengths[0]*sizeof(float));
...
static void onlineClusterKNN_free(t_onlineClusterKNN *x)
{
    ...
    for(i=0; i<x->numClusters; i++)
		t_freebytes_(x->means[i].instance, sizeof(float));
    }
	...
	t_freebytes_(x->featureInput, x->featureLength*sizeof(float));
...
static void *onlineClusterKNN_new(t_symbol *s, long argc, t_atom *argv)
{
	...
	if ((x = (t_onlineClusterKNN *)object_alloc(onlineClusterKNN_class))) {
        ...
        x->featureInput = (float *)t_getbytes_(0);
