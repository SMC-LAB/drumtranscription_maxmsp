...
int tIDLib_getBarkFilterSpacing(t_float **filterFreqs, int oldSizeFilterFreqs, t_float spacing, t_float sr)
{
	...
	(*filterFreqs) = (t_float *)t_resizebytes_((*filterFreqs), oldSizeFilterFreqs*sizeof(t_float), 0);

	while(sumFreq<=sr*0.5)
	{
		(*filterFreqs) = (t_float *)t_resizebytes_((*filterFreqs), sizeFilterFreqs*sizeof(t_float), (sizeFilterFreqs+1)*sizeof(t_float));
...
void tIDLib_createFilterbank(t_float *filterFreqs, t_filter **filterbank, int oldNumFilters, int newNumFilters, t_float window, t_float sr)
{
	...
	// create local memory
	binFreqs = (t_float *)t_getbytes_(windowHalfPlus1*sizeof(t_float));

	// free memory for each filter
	for(i=0; i<oldNumFilters; i++)
		t_freebytes_((*filterbank)[i].filter, (*filterbank)[i].size*sizeof(t_float));

	(*filterbank) = (t_filter *)t_resizebytes_((*filterbank), oldNumFilters*sizeof(t_filter), newNumFilters*sizeof(t_filter));
	...
	for(i=1; i<=newNumFilters; i++)
	{
		...
		// grab memory for this filter
 		...
 		(*filterbank)[i-1].filter = (t_float *)t_getbytes_((*filterbank)[i-1].size*sizeof(t_float));
	...
	// free local memory
	t_freebytes_(binFreqs, windowHalfPlus1*sizeof(t_float));
}
...
void tIDLib_filterbankMultiply(t_sample *spectrum, int normalize, int filterAvg, t_filter *filterbank, int numFilters)
{
	...
	// create local memory
	filterPower = (t_float *)t_getbytes_(numFilters*sizeof(t_float));
	...
	// free local memory
	t_freebytes_(filterPower, numFilters*sizeof(t_float));
}
