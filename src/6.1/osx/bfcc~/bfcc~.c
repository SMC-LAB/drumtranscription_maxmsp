...
// method prototypes
static void bfcc_tilde_bang(t_bfcc *x);
static void bfcc_tilde_window(t_bfcc *x, int w);
static void bfcc_tilde_overlap(t_bfcc *x, int o);
static void bfcc_tilde_windowFunction(t_bfcc *x, int f);
static void bfcc_tilde_powerSpectrum(t_bfcc *x, int spec);
static void bfcc_tilde_create_filterbank(t_bfcc *x, t_float bs);
static void bfcc_tilde_filterFreqs(t_bfcc *x);
static void bfcc_tilde_print(t_bfcc *x);
static void bfcc_tilde_hat(t_bfcc *x, int filt);
static void bfcc_tilde_normalize(t_bfcc *x, int norm);
static void bfcc_tilde_filter_avg(t_bfcc *x, int avg);

void *bfcc_new(t_symbol *s, long argc, t_atom *argv);
void bfcc_free(t_bfcc *x);
void bfcc_dsp(t_bfcc *x, t_signal **sp, short *count);
void bfcc_dsp64(t_bfcc *x, t_object *dsp64, short *count, double samplerate, long maxvectorsize, long flags);
t_int *bfcc_perform(t_int *w);
void bfcc_perform64(t_bfcc *x, t_object *dsp64, double **ins, long numins, double **outs, long numouts, long sampleframes, long flags, void *userparam);


// global class pointer variable
static t_class *bfcc_class = NULL;
...
{	
	...
	t_class *c = class_new("bfcc~", (method)bfcc_new, (method)dsp_free, (long)sizeof(t_bfcc), 0L, A_GIMME, 0);
    
    class_dspinit(c);
	class_register(CLASS_BOX, c);
	bfcc_class = c;
	
	class_addmethod(c, (method)bfcc_dsp,		"dsp",		A_CANT, 0);		// Old 32-bit MSP dsp chain compilation for Max 5 and earlier
	class_addmethod(c, (method)bfcc_dsp64,		"dsp64",	A_CANT, 0);		// New 64-bit MSP dsp chain compilation for Max 6
	//class_addmethod(c, (method)bfcc_assist,	"assist",	A_CANT, 0);
    
    
	addbang((method)bfcc_tilde_bang);
    
    class_addmethod(bfcc_class, 
                    (method)bfcc_tilde_bang,
                    "bang",
                    0
                    ); 
    
    class_addmethod(bfcc_class, 
                    (method)bfcc_tilde_filterFreqs,
                    "filter_freqs",
                    0
                    ); 
    
    class_addmethod(bfcc_class, 
                    (method)bfcc_tilde_print,
                    "print",
                    0
                    );     
	...
	class_addmethod(
                    bfcc_class,
                    (method)bfcc_tilde_windowFunction,
                    "window_function",
                    A_DEFLONG,
                    0
                    );
	...
	class_addmethod(
                    bfcc_class,
                    (method)bfcc_tilde_powerSpectrum,
                    "power_spectrum",
                    A_DEFLONG,
                    0
                    );
    ...
    class_addmethod(
                    bfcc_class,
                    (method)bfcc_tilde_create_filterbank,
                    "filterbank",
                    A_DEFFLOAT,
                    0
                    );
...
void *bfcc_new(t_symbol *s, long argc, t_atom *argv)
{
	t_bfcc *x = (t_bfcc *)object_alloc(bfcc_class);
    int i, isPow2;
    double timef = 0.0;
	//s=s;
...
static void bfcc_tilde_bang(t_bfcc *x)
{
	...
	for(i=0; i<x->numFilters; i++)
		atom_setfloat(listOut+i, x->bfcc[i]);
...
static void bfcc_tilde_create_filterbank(t_bfcc *x, t_float bs)
{
	...
	x->bfcc = (t_float *)t_resizebytes_(x->bfcc, oldNumFilters*sizeof(t_float), x->numFilters*sizeof(t_float));
...
void bfcc_dsp(t_bfcc *x, t_signal **sp, short *count)
{
	...
	dsp_add(bfcc_perform, 4, x, sp[0]->s_vec, sp[1]->s_vec, sp[0]->s_n);
...
void bfcc_dsp64(t_bfcc *x, t_object *dsp64, short *count, double samplerate, long maxvectorsize, long flags)
{
	...
	object_method(dsp64, gensym("dsp_add64"), x, bfcc_perform64, 0, NULL);
